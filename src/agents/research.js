...
- Articles must be published within the LAST 30 DAYS (February-March 2026). Reject anything older.
...
- Do NOT suggest the same articles you found in previous searches

For each article, provide a brief summary of what it covers.

Respond with ONLY a JSON array:
[{"title":"Original article title","summary":"2-3 sentence summary","source_name":"Website name","source_url":"https://full-article-url","date":"2026-03-15","content_type":"recipe|ingredient|health|tip|news|story"}]`
  }], { maxTokens: 2000 });

  console.log('[Research] Discover raw length:', (raw||'').length, 'preview:', (raw||'').slice(0, 300));
  
  if (!raw || raw.length === 0) {
    throw new Error('Empty response from search. Check API key and rate limits.');
  }
  ...
  try {
    ...
    console.error('[Research] Full raw response:', raw.slice(0, 1000));
    // Fallback: try to get Claude to extract JSON
    try {
      const extracted = await callClaude([{
        role: 'user',
        content: 'Extract ONLY the JSON array from this text. Return NOTHING else, just the JSON starting with [ and ending with ]:\n\n' + raw.slice(0, 3000)
      }], { maxTokens: 1500, model: 'claude-haiku-4-5-20251001' });
      return parseJSON(extracted);
    } catch {
      throw new Error('Could not parse discover results');
    }
  }
}
