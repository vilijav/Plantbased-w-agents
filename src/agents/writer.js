...
3. Write a UNIQUE intro — pick a DIFFERENT approach than "Stojite u prodavnici". Use one of: surprising fact, misconception correction, direct authority statement, seasonal connection, or vivid mini-story.
...
6. If a YouTube video is the source, LINK to it in the article and mention the creator by name

CRITICAL RULES:
- Do NOT invent any facts. Everything must come from the sources below.
- Do NOT add health claims that aren't in the sources.
- LINKS: Use sparingly. Max 3-4 <a href> links total. Link the original source, a key study, and video/Reddit if relevant.
- Use <strong> for bold (never markdown **)
- No em-dashes (—)
- IZVORI SECTION at end must be a proper HTML list:
  <h2>Izvori</h2>
  <ul>
  <li><a href="URL">Source name</a></li>
  </ul>
  Every source URL must be in this list as a clickable link.
