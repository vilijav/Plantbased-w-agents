...
export async function fullReview(article, lang = 'sr') {
  ...
  if (lang === 'sr') {
    const { fixCroatianWords, fixFormatting } = await import('../utils/serbian-fixer.js');
    html = fixCroatianWords(html);
    html = fixFormatting(html);
    finalTitle = fixCroatianWords(finalTitle);
    finalMeta = fixCroatianWords(finalMeta);
  }
