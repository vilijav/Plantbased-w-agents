...
export async function generateArticle({ title, angle, sourceUrl = null, lang = 'sr', category = 'Top Namirnice', length = 'medium', tone = 50, contentType = '' }) {
  console.log(`\n${'='.repeat(60)}`);
  ...
  try {
    ...
    const draft = await writeArticle({ title, angle, sources, lang, category, length, tone, contentType });
    await sleep(12000);
